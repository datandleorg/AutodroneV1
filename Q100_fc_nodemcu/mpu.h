 ...
 * I make the mean of 200 values, that should be enough*/
  if(acc_error==0)
  {
    for(int a=0; a<200; a++)
    {
      ...
      Acc_rawX=(wire_read_16()) * (1.0f / 4096.0f) ; //each value needs two registres
      Acc_rawY=(wire_read_16()) * (1.0f / 4096.0f) ;
      Acc_rawZ=(wire_read_16()) * (1.0f / 4096.0f) ;
 ...
 * I make the mean of 200 values, that should be enough*/
  if(gyro_error==0)
  {
    for(int i=0; i<200; i++)
    {
      ...
      Gyr_rawX=(wire_read_16()) * (1.0f / 32.8f);     //Once again we shif and sum
      Gyr_rawY=(wire_read_16()) * (1.0f / 32.8f);
      Gyr_rawZ=(wire_read_16()) * (1.0f / 32.8f);
...
void read_IMU(){
  ...
  Acc_rawX = (wire_read_16()) * (1.0f / 4096.0f);                               //Add the low and high byte to the acc_x variable.
  Acc_rawY = (wire_read_16()) * (1.0f / 4096.0f);                               //Add the low and high byte to the acc_y variable.
  Acc_rawZ = (wire_read_16()) * (1.0f / 4096.0f);                               //Add the low and high byte to the acc_z variable.
  temperature = wire_read_16();                               //Add the low and high byte to the temperature variable.
  Gyr_rawX = (wire_read_16()) * (1.0f / 32.8f);                             //Read high and low part of the angular data.
  Gyr_rawY = (wire_read_16()) * (1.0f / 32.8f);                             //Read high and low part of the angular data.
  Gyr_rawZ = (wire_read_16()) * (1.0f / 32.8f);                             //Read high and low part of the angular data.
